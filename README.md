...
**Part 1 due: Monday, January 13th 11:59PM**

**Due: Wednesday, January 22nd 11:59 PM**
...
```sh
$ git clone git@gitlab.cs.washington.edu:cse444-25wi/simple-db-MY_GITLAB_USERNAME.git
...
$ git remote add upstream git@gitlab.cs.washington.edu:cse444-25wi/simple-db.git
```
...
```sh
$ git remote -v
  origin  git@gitlab.cs.washington.edu:cse444-25wi/simple-db-username.git (fetch)
  origin  git@gitlab.cs.washington.edu:cse444-25wi/simple-db-username.git (push)
  upstream    git@gitlab.cs.washington.edu:cse444-25wi/simple-db.git (fetch)
  upstream    git@gitlab.cs.washington.edu:cse444-25wi/simple-db.git (push)
```
...
To git@gitlab.cs.washington.edu:cse444-25wi/simple-db-username.git
...
From gitlab.cs.washington.edu:cse444-25wi/simple-db
...
[Here is a guide to installing Ant in case you are having trouble!](https://docs.google.com/document/d/131uUYw26hrycs4DlKqM08Ydbw4f7odWIeqZtmEnOp6Y/edit?usp=sharing)
...
We also created a [detailed guide on setting up IntelliJ for SimpleDB (click here)](https://docs.google.com/document/d/1TQrz2WX_qyRsz2wNrHHCqWnWeENfhWwLkNPOu2hWe6g/edit?usp=sharing)
...
After executing the `turnInLab.sh` script, make sure to check your repository on GitLab to ensure that the tag has been property applied!  You may do this by visiting `https://gitlab.cs.washington.edu/cse444-25wi/simple-db-YOUR_USERNAME/tags`.
...
To git@gitlab.com:cse444-25wi/hw-answers-pirateninja.git
 - [deleted]         lab1
...
```sh
$ git clone git@gitlab.cs.washington.edu:cse444-25wi/simple-db-username.git confirmation_directory
...
**Important:** before testing, we will replace your `build.xml` and the entire contents of the `test` directory with our version of these files. This means you cannot change the format of `.dat` files! You should also be careful changing our APIs.  You should test that your code compiles the unmodified tests. In other words, during the grading process we will clone your repository, replace the files mentioned above, compile it, and then grade it. It will look roughly like this:
...
An additional 10% of your grade will be based on the quality of your writeup and our subjective evaluation of your code.
